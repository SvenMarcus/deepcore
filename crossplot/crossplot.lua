...
require("deepcore/crossplot/context")

---A module that allows publish-subscribe communication between different Lua script environments
crossplot = { __instance = nil, __important = true, __batch_processors = {}, __contexts = {} }
...
function crossplot:main()
	...
	DebugMessage("%s -- initializing MainKeyValueStoreBasedEventBus", tostring(Script))
...
---Acquire a crossplot context with a ready condition
---@return CrossplotContext
function crossplot:acquire_context(ready_condition)
	if not self.__instance then
		DebugMessage("crossplot has not been initialized in this plot")
		return NULL_CONTEXT
	end

	local context = CrossplotContext(ready_condition, self.__instance)
	self.__contexts[ready_condition] = context
	return context
end
...
function crossplot:update()
	...
	---@type CrossplotContext
	for _, context in pairs(self.__contexts) do
		context:update()
	end
